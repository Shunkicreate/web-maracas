  ...
  return (
    <Menu colorScheme={bg}>
      <MenuButton
        ...
        height={"fit-content"}
      >
        <h1 className="py-4 px-2">{modeTextState}</h1>
      </MenuButton>
      <MenuList
        color={fontColor}
        border={fontColor}
      ...
      >
        {menuTextList.map((text, i) => (
          <MenuItem
            key={i}
            onClick={() => setModeTextState(text)}
            background={bg}
          >
            <Link href={menuTextLink[i]}><h2 className="">{text}マラカス</h2></Link>
          </MenuItem>
        ))}
        <MenuItem
          onClick={() => setModeTextState("トップページ")}
          background={bg}
        >
