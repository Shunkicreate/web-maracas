...
const VolumeButton: React.FC<VolumeButtonProps> = memo(
    ...
    return (
      ...
      >
        {colorModeMemo === "dark" && (
          <>
            {volumeMode === "mute" && (
              <Image src={muteNight} alt="消音画像"></Image>
            )}
            {volumeMode === "low" && (
              <Image src={lowNight} alt="小音量画像"></Image>
            )}
            {(volumeMode === "high" || volumeMode === undefined) && (
              <Image src={highNight} alt="大音量画像"></Image>
            )}
          </>
        )}
        {colorModeMemo === "light" && (
          <>
            ...
            {(volumeMode === "high" || volumeMode === undefined) && (
              <Image src={high} alt="大音量画像"></Image>
            )}
          </>
        )}
      </button>
  ...
  (prevProps, nextProps) => {
    ...
    return (
      getVolumeMode(prevProps.Volume, prevProps.isMuted) ===
        getVolumeMode(nextProps.Volume, prevProps.isMuted) &&
